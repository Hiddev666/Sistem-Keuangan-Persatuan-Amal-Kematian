@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:gap-2 sm:items-center sm:justify-between">
            ...
            <div>
                <span class="inline-flex rtl:flex-row-reverse shadow-sm rounded-md bg-red-800">
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white bg-green-800 border border-gray-300 cursor-default leading-5 ">{{ $page }}</span>
                                    </span>
                                @else
