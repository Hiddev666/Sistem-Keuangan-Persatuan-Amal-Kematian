...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold">{{ isset($donation) ? "Edit Donasi" : "Tambahkan Donasi" }}</h1>
        ...
        <div class="mt-5">
            ...
            @enderror
            @if (isset($donation))
                <form action="{{ route("admin_sumbangan_update")}}" method="post">
                    @csrf
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <div class="hidden">
                                <label for="member_id" class="text-gray-600 text-sm">Anggota</label>
                                <input type="text" id="id" name="id" value="{{ old('id') ?? $donation->id}}"
                        ...
                        <div>
                            ...
                            <select name="status" id="status"
                                ...
                                @if (isset($donation->status))
                                    <option value="{{ $donation->status }}">{{ ucfirst($donation->status) }}</option>
                                @endif
            ...
            @else
                <form action="{{ route("admin_sumbangan_create")}}" method="post" id="form_add">
                    @csrf
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <div id="input-anggota-kas">
                        ...
                        if (data.length === 0) {
                            html = `
                                                <div class="border border-gray-300 bg-gray-100 p-2 flex items-center gap-5 justify-between rounded">
                                                    <div>
                                                        <p>${input.value}</p>
                                                        <p class="text-xs text-gray-600">Tambahkan Donatur Non Anggota</p>
                                                        </div>
                                                                <button
                                                                type='button'
                                                                    onclick="test('Non Anggota', '${input.value}')"
