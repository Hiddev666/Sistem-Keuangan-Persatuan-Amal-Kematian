...
@section('content')
    <div class="mb-6">
        ...
        <div class="mt-5 flex flex-col gap-9">
            ...
            <div>
                ...
                <select name="head_member_id" id="head_member_id"  class="px-3 py-2 text-sm w-full border border-gray-300 rounded-md bg-white" required>
                @foreach ($family_card->members as $member)
                    <option value="{{ $member->id }}" {{ $family_card->head_member_id == $member->id ? "selected" : "" }}>{{ $member->id }} - {{ $member->name }}</option>
                @endforeach
                </select>
            </div>
            @endif
            ...
            @if (isset($family_card))
            <form action="{{ route("admin_keanggotaan_anggota_update") }}" class="p-3 rounded-md border border-gray-300 bg-gray-50" id="form_anggota">
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex flex-col gap-2" id="member_form">
                        @foreach ($family_card->members as $member)
                            <div class="grid grid-cols-4 gap-3 border border-gray-200 p-2 bg-white rounded">
                                <div>
                                ...
                                <div>
                                    ...
                                    <select name="status[]" id="status"  class="px-3 py-2 text-sm w-full border border-gray-300 rounded-md bg-white" required>
                                        @foreach ($statuses as $status)
                                            <option value="{{ $status["value"] }}" {{ $status["value"] == $member->status ? "selected" : ""}}>{{ $status["text"] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                    ...
                    @if (auth()->user()->role == "admin" || auth()->user()->role == "ketua")
                    <button type="submit"
                            class="w-fit mt-3 px-8 py-2 flex items-center gap-1 bg-green-800 text-sm font-semibold rounded-md text-white transition-all duration-200 ease-in-out hover:bg-green-700">
                            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5">
                                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                <g id="SVGRepo_iconCarrier">
    ...
    <script>
        const familyCardId = document.getElementById("id")
        ...
        formAnggota.addEventListener("submit", (e) => {
            ...
            const result = memberIds.map((member_id, index) => ({
                member_id: member_id,
                name: names[index],
                phone: phones[index],
                status: statuses[index],
                family_card_id: familyCardId.value
            }));

            fetch('/admin/keanggotaan/anggota/update', {
                method: 'POST',
                headers: {
