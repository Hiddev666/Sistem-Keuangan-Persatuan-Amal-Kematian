                ...
                <tbody id="result">
                    @foreach ($contributions as $contribution)
                                <tr class="border-b border-gray-200 transition-all ease-in-out">
                                    <td class="font-normal p-2 text-left font-semibold">{{ $contribution->family_card_id }}</td>
                                    <td class="font-normal p-2 text-left font-semibold">{{ $contribution->family_card->head->name }}</td>
                                    <td class="font-normal p-2 text-left font-medium">{{ rupiah($contribution->amount) }}</td>
    ...
    <script>
        ...
        input.addEventListener('keyup', function () {
            ...
            timeout = setTimeout(() => {
                ...
                fetch(`{{ route('admin_kas_search') }}?q=${encodeURIComponent(keyword)}&period=${period}`)
